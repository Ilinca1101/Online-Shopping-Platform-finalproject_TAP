...
namespace Proiect1.Services
{
    public class ShoppingCartService : IShoppingCartService
    {
        ...
        public async Task<CartItemDto> UpdateQuantity(CartItemQuantUpdateDto cartItemQuantUpdateDto)
        {
            try
            {
                var jsonRequest = JsonConvert.SerializeObject(cartItemQuantUpdateDto);
                var content=new StringContent(jsonRequest, Encoding.UTF8, "application/json-patch+json");

                var response = await httpClient.PatchAsync($"api/ShoppingCart/{cartItemQuantUpdateDto.CartItemId}", content);
